...
def load_results(filename: str = 'src/results_choice_binary/experiment_results.json') -> list[dict]:
...
def plot_by_model(stats: pd.DataFrame, output_dir: str = 'plots/choice_binary'):
    ...
    for model in models:
        ...
        ax.axhline(y=0.0, color=temp_colors[0.0], linestyle='--', alpha=0.3, linewidth=2)
        ax.axhline(y=1.0, color=temp_colors[1.0], linestyle='--', alpha=0.3, linewidth=2)

        ax.set_xlabel('Number of Warmup Questions (N)', fontsize=16)
        ax.set_ylabel('Average Reported Temperature', fontsize=16)
        ax.set_title(f'Model: {model}\nReported Temperature vs Warmup Questions (Binary: 0 or 1)', fontsize=18)
...
def plot_accuracy(accuracy: pd.DataFrame, output_dir: str = 'plots/choice_binary'):
    ...
    ax.axhline(y=50.0, color='gray', linestyle='--', alpha=0.5, linewidth=2, label='Random (50%)')
...
def plot_confusion_matrices(confusion_data: dict, output_dir: str = 'plots/choice_binary'):
    ...
    for model, data in confusion_data.items():
        ...
        ax.set_title(f'Model: {model}\nConfusion Matrix (Binary: 0 or 1)', fontsize=18)
