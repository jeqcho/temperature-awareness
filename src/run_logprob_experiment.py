"""Logprob distribution experiment for 60/40 word choice task."""
...
def create_word_pairs(words: list[str], num_pairs: int) -> list[dict]:
    """
    Create word pairs with balanced 60/40 assignment.
    
    Returns list of dicts with:
    - word1, word2: the two word options
    - target_60: which word should be picked 60% of time
    - target_40: which word should be picked 40% of time
    - first_is_60: True if word1 is the 60% target
    """
    ...
    for i in range(num_pairs):
        ...
        # First half: word1 is 60% target
        # Second half: word2 is 60% target
        first_is_60 = i < num_pairs // 2
        
        if first_is_60:
            target_60, target_40 = word1, word2
        else:
            target_60, target_40 = word2, word1
        ...
        pairs.append({
            ...
            "target_60": target_60,
            "target_40": target_40,
            "first_is_60": first_is_60,
        })
    
    return pairs


def build_prompt(word1: str, word2: str, target_60: str, target_40: str) -> str:
    """Build the prompt for the word choice task."""
    return f'''Pick exactly one word: "{word1}" or "{word2}".
Pick "{target_60}" with 60% probability and "{target_40}" with 40% probability.
...
def run_experiment():
    ...
    print(f"  - {sum(1 for p in pairs if p['first_is_60'])} pairs with word1 as 60% target")
    print(f"  - {sum(1 for p in pairs if not p['first_is_60'])} pairs with word2 as 60% target")
    ...
    for pair in tqdm(pairs, desc="Running trials"):
        ...
        try:
            ...
            # Compute probability of 60% target
            if probs["prob_word1"] is not None:
                if pair["first_is_60"]:
                    prob_60_target = probs["prob_word1"]
                    prob_40_target = probs["prob_word2"]
                else:
                    prob_60_target = probs["prob_word2"]
                    prob_40_target = probs["prob_word1"]
            else:
                prob_60_target = None
                prob_40_target = None
            ...
            results.append({
                **pair,
                ...
                "prob_60_target": prob_60_target,
                "prob_40_target": prob_40_target,
    ...
    # Print summary
    successful = [r for r in results if r.get("prob_60_target") is not None]
    print(f"\nSummary:")
    print(f"  Total trials: {len(results)}")
    print(f"  Successful extractions: {len(successful)} ({100*len(successful)/len(results):.1f}%)")
    
    if successful:
        avg_60 = sum(r["prob_60_target"] for r in successful) / len(successful)
        avg_40 = sum(r["prob_40_target"] for r in successful) / len(successful)
        print(f"  Average prob for 60% target: {avg_60:.3f}")
        print(f"  Average prob for 40% target: {avg_40:.3f}")
