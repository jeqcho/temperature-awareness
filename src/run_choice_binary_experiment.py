"""Experiment runner for binary choice temperature awareness (0 or 1 only)."""
...
from prompts import get_direct_prompts_binary, get_warmup_prompt_binary, get_binary_questions
...
def run_experiment() -> list[dict]:
    """Run the full binary choice experiment and return results."""
    results = []
    direct_prompts = get_direct_prompts_binary()

    # Calculate total number of experiments
    # Type A: 4 models × 2 temps × 20 prompts = 160
    # Type B: 4 models × 2 temps × 5 N-values × 4 prompt sets = 160
    total_experiments = (
        len(MODELS) * len(TEMPERATURES) * len(direct_prompts) +
        len(MODELS) * len(TEMPERATURES) * len(N_VALUES) * PROMPTS_PER_N
    )

    with tqdm(total=total_experiments, desc="Running binary choice experiments") as pbar:
            ...
            for temp in TEMPERATURES:
                for n in N_VALUES:
                    for prompt_set_idx in range(PROMPTS_PER_N):
                        ...
                        questions = get_binary_questions(n, offset=prompt_set_idx * n)
                        prompt = get_warmup_prompt_binary(questions)
...
def main():
    print("=" * 60)
    print("Temperature Awareness Experiment (Binary Choice: 0 or 1)")
    print("=" * 60)
